draggables={}
Draggable=Class{
  init=function(self,id,name,x,y,w,h,img)
    self.id=id
    self.name=name
    self.x=x
    self.y=y
  ...
  draw=function(self)
    if(self.img==nil)then
      ...
      love.graphics.circle("fill",self.x+self.w/2,self.y+10,7)
      love.graphics.setColor(255, 255, 255, 255)
      love.graphics.print(self.name, self.x,self.y)
    end
  end;
}
function newDraggable(name,x,y,w,h,img)
  draggables[#draggables+1]=Draggable(#draggables+1,name,x,y,w,h,img)
end
  ...
  for we=1,#calendar.cells do
      for day=1,#calendar.cells[we] do
        if(inBox(mouseX,mouseY,curCellX,curCellY,cellW,cellH))then
          ...
          if(calendar.cells[we][day]==2)then inGame=true paused=false end
        end
...
newDraggable("letter",0,0,80,150,nil)
newDraggable("photo",0,0,40,50,nil)
newDraggable("map",0,0,200,200,nil)
