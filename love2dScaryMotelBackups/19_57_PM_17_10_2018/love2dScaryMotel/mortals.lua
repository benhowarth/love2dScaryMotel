...
Mortal=Class{
  ...
  update=function(self)
    ...
    elseif(self.state=="STAYING")then
      ...
      elseif(self.type=="murderer")then
        --murder a person?
        if(math.random()>0.8 and everyMS(100))then
  ...
  getFinalReview=function(self,fragNumber)
    ...
    if(#self.reviewFragsNeg>0 and #self.reviewFragsPos==0)then
      negNum=clamp(#self.reviewFragsNeg,0,fragNumber)
      posNum=0
    elseif(#self.reviewFragsNeg==0 and #self.reviewFragsPos>0)then
      negNum=0
      posNum=clamp(#self.reviewFragsNeg,0,fragNumber)
    else
      --negNum=clamp(fragNumber-math.floor(love.math.random(0,fragNumber)),0,#self.reviewFragsNeg)
      --fragNumber=clamp(fragNumber,1,#self.reviewFragsNeg)
      --if(#self.reviewFragsNeg<#self.reviewFragsPos)then
      --  fragNumber=clamp(fragNumber,1,#self.reviewFragsNeg)
      --else
      --  fragNumber=clamp(fragNumber,1,#self.reviewFragsPos)
      --end
      local rMax=fragNumber
      local rvn =#self.reviewFlagsNeg
      local rvp =#self.reviewFlagsPos
      if(rvn>=3 and rvp>=3)then rMax=fragNumber
      else if(rvp==1)then rMax=rvn
      else if(rvn==1)then rMax=rvp
      else if(rvn==2 and rvn==2)then rMax=2
      else if(rvn==2 and rvn==3)then rMax=2
      else if(rvn==3 and rvn==2)then rMax=2
      end
      r=love.math.random(0,rMax)
      --love.window.showMessageBox("fragNum", Inspect(fragNumber).."/"..Inspect(r), "info", true)
      negNum=fragNumber-r
      --negNum=clamp(#self.reviewFragsNeg-math.floor(love.math.random)
      posNum=fragNumber-negNum
    end
    ...
    if(#self.reviewFragsNeg~=0 or #self.reviewFragsPos~=0)then
      ...
      for i=1,posNum do
        ...
        self.rating=((self.rating*(i+negNum-1))+self.reviewFragsPos[i].rating)/i
      end
