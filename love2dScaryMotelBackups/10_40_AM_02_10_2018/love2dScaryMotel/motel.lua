...
M.print=function(x,y,w,fW,fH)
  ...
  love.graphics.rectangle("fill",x-100,y+floors[1].height-floors[1].lineWidth*2,window.w+100,window.h/2)
...
newProblem("No Vending Machine",nil,true,nil,100,nil,nil,{"There was nothing to eat"},1,nil,function(self,x,y,w) love.graphics.draw(M.imgs.vendingMachine,x,y,0,w,w,M.imgs.vendingMachine:getWidth()/2,M.imgs.vendingMachine:getHeight()/2) end,{"It had a vending machine"},3)

newProblem("No Hotdogs",nil,true,function() return isBought("Vending Machine") end,100,nil,nil,{"There was only vending machine to eat"},3,nil,nil,{"It had delicious hotdogs"},4)
newUpgrade("Vending Machine",nil,10,nil,{"No Vending Machine"},{"No Hotdogs"})
...
function(self)
  if(getNightTime() and #cats<10)then
    --if(math.floor(math.fmod(time2,dayLen/3))==0)then
      ...
      --cats[#cats+1]=Cat(0,0)
      cats[#cats+1]=Cat(window.w-math.random(40,500),floors[1].height+math.random(0,15))
      love.window.showMessageBox("new cat"..Inspect(#cats), Inspect(cats), "info", true)
    --end
  end
  if(inBox(mouseX,mouseY,window.w/2,-150+window.h/2,100,100) and love.mouse.isDown(1) and hotdogClosedTimer==0) then
    cats={}
    evilPercAdd(0.001)
    hotdogClosedTimer=5
    hotdogTop=math.ceil(math.random()*#mortalTop)
...
function(self,x,y,w)
  --love.graphics.draw(M.imgs.cat,x,y,0,w,w,M.imgs.cat:getWidth()/2,M.imgs.cat:getHeight()/2)
    ...
    love.graphics.rectangle("line", window.w/2,-150+window.h/2,100,100)
    love.graphics.setColor(255, 255, 255, 255)
...
function(self)
  ...
  if(statueHelpTimer==0)then
    if(statueHelpCounter<1)then
      newNewsStory("A helpful message!","That shiny new statue can be used to eradicate a bad review!",#newsFeed+1)
    elseif(statueHelpCounter<2)then
      newNewsStory("A friendly reminder!","You seem to be having problems with your reviews, try removing a review with the statue!",#newsFeed+1)
    elseif(statueHelpCounter<3)then
      newNewsStory("A firm reminder.","This wouldn't be going so badly if you'd have used the statue.",#newsFeed+1)
    else
      newNewsStory("GIVE IN TO THE STATUE","SUBMIT TO THE STATUE",#newsFeed+1)
    end
  ...
  print=function(self,x,y,w,fW,fH)
    ...
    if(roomMenuId~=nil)then
      ...
      local mY=love.mouse.getY()
   	  ...
   	  roomMenuNameH=drawTextInBox(mortals[self.rooms[roomMenuId].currentMortal].name,fW,fH,mX,mY,150)
      roomMenuInfoH=drawTextInBox(mortals[self.rooms[roomMenuId].currentMortal].bio,fW,fH,mX,mY+10+roomMenuNameH,150)
      roomMenuH=roomMenuNameH+10+roomMenuInfoH+20
      love.graphics.setColor(0,0,0,255)
      love.graphics.rectangle("fill", mouseX,mouseY,150,roomMenuH)
      love.graphics.setColor(255,255,255,255)
      love.graphics.rectangle("line", mouseX,mouseY,150,roomMenuH)
      drawTextInBox(mortals[self.rooms[roomMenuId].currentMortal].name,fW,fH,mouseX,mouseY,150)
      drawTextInBox(mortals[self.rooms[roomMenuId].currentMortal].bio,fW,fH,mouseX,mouseY+10+roomMenuNameH,150)
    end
  end;
}
...
addFloor=function(cost,roomNo)
  ...
  newProblem(string.format("Dirty F%d",#floors),floorNo,true,nil,100,nil,nil,{"It was #veryDirty#"},2,nil,nil,{"It was clean"},4)
  newUpgrade(string.format("Clean F%d",#floors),floorNo,10,nil,{string.format("Dirty F%d",#floors)},nil)
  if(#floors>=2)then
