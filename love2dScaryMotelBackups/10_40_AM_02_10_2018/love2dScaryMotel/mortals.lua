...
Mortal=Class{
  init=function(self,id,floorId)
    ...
    if(self.roomId~=nil)then
      ...
      self.target={x=floors[self.floorId].rooms[self.roomId].x+(newsFeedWidth+125)+roomWidth/2,y=floors[self.floorId].rooms[self.roomId].y+230+motelYOffset-((floorId-1)*floors[1].height)}
    end
    ...
    if(self.state=="ENTERING")then
      if(isRoomAccessible(self.floorId)) then
        ...
        --if(self.x>newsFeedWidth+80)then
          self.state="STAYING"
        elseif(within(self.x,newsFeedWidth+125,10) and not(within(self.y,self.target.y,20)))then
          self.y=self.y-3
          --love.window.showMessageBox(self.name, "climns", "info", true)
        else
  ...
  draw=function(self,xOffset,yOffset)
    if(self.state~="LEFT" and self.state~="STAYING" and self.state~="MISSING")then
      love.graphics.setColor(255, 255, 255, 255)
...
newMortal=function()
  if(getAvailableFloor()~=nil and floors[getAvailableFloor()]:getVacantRoom()~=nil)then
    mortals[#mortals+1]=Mortal(#mortals+1,getAvailableFloor())
  end
end;
